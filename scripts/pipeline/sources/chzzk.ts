...
async function requestPage(http: HttpClient, url: string, headers: Record<string, string>, retryLimit: number): Promise<ChzzkLivesResponse> {
...
export async function fetchChzzk(input: ChzzkFetchInput, http: HttpClient): Promise<ChzzkRawSnapshot> {
  ...
  const retryLimit = Math.max(0, input.retryLimit);
  let cursor: string | undefined;
  let pages = 0;

  while (pages < pageLimit) {
    const page = await requestPage(http, requestUrl(cursor), headers, retryLimit);
  ...
  return {
    ...
    source: "chzzk",
    request: { endpoint: LIVES_URL, pageLimit, pageSize: PAGE_SIZE, retryLimit },
