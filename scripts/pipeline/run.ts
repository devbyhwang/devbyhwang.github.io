...
import type { IgdbRawSnapshot, PipelineOptions, PipelineResult, RawResponseRecorder, RawSources, SourceAdapters, SourceConfig, SteamRawSnapshot, TwitchRawSnapshot } from "./model";
...
async function fetchAllSourcesWithStatus(
  ...
  const [igdbResult] = await Promise.allSettled([
    adapters.igdb({
      ...
      topIgdbIds: twitch.snapshot.topGames.map((game) => game.igdbId),
...
export async function runPipeline(options: PipelineOptions): Promise<PipelineResult> {
  ...
  const games = enrichGames(joined, knowledge, asOf).map((game) => ({
    ...
    buzz: {
      ...game.buzz,
      viewerGrowth7d: twitchFresh ? growth7d(nextHistory, game.id, asOf) : null,
    },
  }));
  const fileStore = createNodeFileStore(rootDir);
  const emitted = emitCatalog({ generatedAt: asOf, games }, readCatalog(fileStore), fileStore);
