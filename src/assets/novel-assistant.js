    ...
    const messages = {
      ko: {
        ...
        analysisSetupTitle: "분석 설정",
        analysisSetupHint: "평가 기준은 노드에서 자동 생성되고, 작품 맥락과 피드백 방식만 직접 조정합니다.",
        checksCardTitle: "평가 기준",
        checksCardSummary: "{count}개 루브릭 자동 생성",
        checksCardHint: "선택 노드 전용 평가 기준입니다. 직접 수정하지 않고 읽기 전용으로 노출합니다.",
        metaCardTitle: "작품 맥락",
        metaCardSummary: "{draftStage} · {genre} · {narrativePOV}",
        presetCardTitle: "피드백 방식",
        presetCardSummary: "{label}",
        autoGenerated: "자동 생성",
        genreLabel: "장르",
        draftStageLabel: "작성 단계",
        narrativePOVLabel: "시점",
        authorGoalLabel: "이번 점검 목표",
        authorGoalPlaceholder: "예: 이번에는 갈등 밀도와 장면 추진력만 점검",
        mustKeepLabel: "반드시 유지할 요소",
        mustKeepPlaceholder: "줄마다 1개씩 입력\n예: 주인공 직업 설정 유지",
        rubricQuestion: "질문",
        rubricLookFor: "주요 확인 사항",
        rubricScoreGuide: "점수 가이드",
        strictLabel: "엄격하게",
        balancedLabel: "균형 있게",
        supportiveLabel: "격려형",
        presetStrictHint: "약점과 누락을 먼저 짚고, 수정 우선순위를 단호하게 제시합니다.",
        presetBalancedHint: "강점은 짧게 인정하되, 개선 우선순위를 분명하게 제시합니다.",
        presetSupportiveHint: "초보자도 받아들이기 쉽게 쓰되, 문제 진단 자체는 흐리지 않습니다.",
        optionGenreUnknown: "미정",
        optionGenreModernFantasy: "현대판타지",
        optionGenreFantasy: "판타지",
        optionGenreRomanceFantasy: "로맨스판타지",
        optionGenreRomance: "로맨스",
        optionGenreThriller: "스릴러",
        optionGenreDrama: "드라마",
        optionGenreAction: "액션",
        optionGenreEtc: "기타",
        optionDraftPlanning: "구상",
        optionDraftFirst: "초고",
        optionDraftRewrite: "개고",
        optionDraftPolish: "퇴고",
        optionPovUnknown: "미정",
        optionPovFirst: "1인칭",
        optionPovThirdLimited: "3인칭 제한",
        optionPovThirdOmni: "3인칭 전지",
      ...
      en: {
        ...
        analysisSetupTitle: "Analysis Setup",
        analysisSetupHint: "Rubrics are auto-generated from the selected node. You only adjust story context and feedback mode.",
        checksCardTitle: "Evaluation Criteria",
        checksCardSummary: "{count} auto-generated rubrics",
        checksCardHint: "These checks are derived from the selected node and shown as read-only analysis criteria.",
        metaCardTitle: "Story Context",
        metaCardSummary: "{draftStage} · {genre} · {narrativePOV}",
        presetCardTitle: "Feedback Mode",
        presetCardSummary: "{label}",
        autoGenerated: "Auto-generated",
        genreLabel: "Genre",
        draftStageLabel: "Draft Stage",
        narrativePOVLabel: "Point of View",
        authorGoalLabel: "Current Review Goal",
        authorGoalPlaceholder: "Example: Focus on conflict density and scene propulsion only",
        mustKeepLabel: "Must-Keep Elements",
        mustKeepPlaceholder: "One item per line\nExample: Preserve the protagonist's job setup",
        rubricQuestion: "Question",
        rubricLookFor: "Look for",
        rubricScoreGuide: "Score guide",
        strictLabel: "Strict",
        balancedLabel: "Balanced",
        supportiveLabel: "Supportive",
        presetStrictHint: "Lead with weaknesses and missing craft moves in a firm tone.",
        presetBalancedHint: "Acknowledge strengths briefly, then clarify revision priorities.",
        presetSupportiveHint: "Keep the tone encouraging, but do not soften the diagnosis itself.",
        optionGenreUnknown: "Undecided",
        optionGenreModernFantasy: "Modern Fantasy",
        optionGenreFantasy: "Fantasy",
        optionGenreRomanceFantasy: "Romance Fantasy",
        optionGenreRomance: "Romance",
        optionGenreThriller: "Thriller",
        optionGenreDrama: "Drama",
        optionGenreAction: "Action",
        optionGenreEtc: "Other",
        optionDraftPlanning: "Planning",
        optionDraftFirst: "First Draft",
        optionDraftRewrite: "Rewrite",
        optionDraftPolish: "Polish",
        optionPovUnknown: "Undecided",
        optionPovFirst: "First Person",
        optionPovThirdLimited: "Third-Limited",
        optionPovThirdOmni: "Third-Omniscient",
    ...
    const editorialPresets = {
      strict: {
        id: "strict",
        labelKey: "strictLabel",
    ...
    function makeScoreGuide(high, mid, low) {
      return { high, mid, low };
    }

    function makeRubric(nodeId, id, dimension, label, question, lookFor, high, mid, low) {
      return {
        id: `${nodeId}__${id}`,
        dimension,
        label,
        question,
        lookFor,
        scoreGuide: makeScoreGuide(high, mid, low),
      };
    }

    const koRubricMap = {
      s1_lock_lead: [
        makeRubric("s1_lock_lead", "reader_bond", "core", "독자 연결력", "주인공이 독자가 따라가고 싶을 만큼 구체적으로 붙는가?", ["동질감·연민·호감 장치가 행동이나 사건으로 제시된다", "주인공 소개가 이력서가 아니라 감정 진입점으로 작동한다"], "독자가 감정적으로 붙을 이유가 선명하다.", "공감 장치는 있으나 장면화가 약하거나 평면적이다.", "주인공이 기능 설명에 머물러 정서적 연결이 거의 없다."),
        makeRubric("s1_lock_lead", "inner_conflict", "pressure", "내적 갈등 압력", "주인공의 상반된 욕망이나 두려움이 실제 행동을 흔드는가?", ["서로 충돌하는 두 목소리가 구체 사건과 연결된다", "갈등 때문에 선택이 지연되거나 왜곡된다"], "내적 갈등이 주인공의 판단과 행동을 강하게 흔든다.", "갈등은 보이지만 행동까지 밀어붙이는 압력은 약하다.", "갈등이 추상 설정에 그쳐 서사적 압력으로 작동하지 않는다."),
        makeRubric("s1_lock_lead", "stakes", "effect", "실패 대가", "주인공이 실패할 때 잃는 것이 충분히 치명적인가?", ["플롯·관계·자아 차원의 손실이 구체적으로 드러난다", "실패 대가가 목표 추구의 필연성을 만든다"], "대가가 선명하고 치명적이라 독자가 응원하게 된다.", "대가는 있으나 규모나 체감 강도가 다소 약하다.", "실패해도 큰일이 없어 보여 목표 추구의 절박함이 떨어진다."),
      ],
      s1_lock_objective: [
        makeRubric("s1_lock_objective", "clarity", "core", "지배 목표 선명도", "주인공의 지배 목표가 한 문장으로 또렷하게 잡히는가?", ["얻거나 벗어나야 할 목표가 가시적으로 표현된다", "추상 욕망이 아니라 행동 가능한 목표다"], "목표가 한 문장으로 명확하고 오해의 여지가 없다.", "목표는 보이지만 추상어가 섞여 선명도가 약하다.", "무엇을 원하는지 흐릿해 장면 추진력이 생기지 않는다."),
        makeRubric("s1_lock_objective", "necessity", "pressure", "필연성과 시간 압박", "왜 지금 이 목표여야 하는지가 충분히 납득되는가?", ["시간 압박이나 되돌릴 수 없는 조건이 제시된다", "지금 움직여야 하는 이유가 인과적으로 이어진다"], "지금 당장 움직여야 할 이유가 강하게 작동한다.", "필연성은 있으나 긴박감이나 명분이 약하다.", "지금 이 목표를 pursue해야 할 이유가 약해 서사가 느슨하다."),
        makeRubric("s1_lock_objective", "story_question", "effect", "스토리 질문 흡인력", "이 목표가 독자를 붙드는 핵심 질문을 만들어내는가?", ["성공 여부가 궁금한 질문이 자연스럽게 생긴다", "실패 시 결과가 독자의 기대를 자극한다"], "독자가 끝까지 확인하고 싶을 질문이 분명하다.", "질문은 있으나 강한 호기심이나 긴장으로 이어지지 않는다.", "목표가 스토리 질문을 만들지 못해 독서 동력이 약하다."),
      ],
      s1_lock_confrontation: [
        makeRubric("s1_lock_confrontation", "opposition_design", "core", "대립 설계", "주인공 목표를 가로막는 반대세력이 분명하게 설계되었는가?", ["주요 반대세력의 성격과 기능이 명확하다", "보조 장애물이 서로 다른 방식으로 압박한다"], "대립 세력이 층위별로 분명하고 기능이 겹치지 않는다.", "대립은 있으나 역할이 겹치거나 단조롭다.", "대립 세력이 약하거나 모호해 갈등 축이 흐려진다."),
        makeRubric("s1_lock_confrontation", "escalation", "pressure", "압박 상승선", "대립이 초반에서 후반으로 갈수록 강해지는가?", ["장애물이 단계적으로 심화된다", "주인공 대응이 점점 불리해진다"], "압박이 점증하며 서사적 긴장을 꾸준히 끌어올린다.", "상승은 있으나 단계 간 차이가 약하다.", "갈등 강도가 평평해 중반 이후 긴장이 처진다."),
        makeRubric("s1_lock_confrontation", "reader_worry", "effect", "독자 걱정 유발", "독자가 실제로 주인공을 걱정할 만큼 대립이 체감되는가?", ["주인공이 쉽게 이길 수 없다는 근거가 있다", "실패 가능성이 구체적으로 드러난다"], "독자가 결과를 불안하게 지켜볼 만큼 위협이 선명하다.", "우려 요소는 있으나 체감 강도가 약하다.", "반대세력이 만만해 보여 긴장과 불안이 생기지 않는다."),
      ],
      s1_lock_knockout: [
        makeRubric("s1_lock_knockout", "decisive_event", "core", "결정타 설계", "결말의 핵심 사건이 스토리 전체를 마무리할 결정타가 되는가?", ["결말 사건이 지배 목표와 직접 연결된다", "클라이맥스 선택이 결말 효과를 만든다"], "결정타가 명확하고 이야기의 정점을 설득력 있게 완성한다.", "핵심 사건은 있으나 여운이나 필연성이 조금 약하다.", "결말 사건이 흐리거나 핵심 갈등과 따로 놀아 타격감이 약하다."),
        makeRubric("s1_lock_knockout", "payoff", "pressure", "질문·복선 회수", "스토리 질문과 주요 복선이 적절히 회수되는가?", ["초반에 던진 질문이 결말에서 응답된다", "회수와 놀라움의 균형이 맞는다"], "질문과 복선이 자연스럽고 만족스럽게 회수된다.", "회수는 되지만 일부가 기계적이거나 약하다.", "핵심 질문이 흐릿하게 끝나 독자가 허전함을 느낀다."),
        makeRubric("s1_lock_knockout", "resonance", "effect", "마지막 울림", "마지막 장면이 감정과 의미를 동시에 남기는가?", ["대가와 감정 후폭풍이 보인다", "마지막 이미지나 행동이 여운을 만든다"], "감정적 타격과 의미적 여운이 함께 남는다.", "울림은 있으나 감정 또는 의미 한쪽이 약하다.", "끝났다는 사실만 남고 여운이나 해석의 잔향이 약하다."),
      ],
      s2_reader_empathy_and_likability: [
        makeRubric("s2_reader_empathy_and_likability", "empathy_trigger", "core", "공감 트리거", "독자가 주인공에게 정서적으로 붙을 장치가 구체적인가?", ["취약성·책임·선의 같은 장치가 사건 속에서 드러난다", "감정 이입 포인트가 설명이 아니라 행동으로 제시된다"], "공감 장치가 자연스럽고 강하게 작동한다.", "공감 포인트는 있으나 직설적 설명에 기대는 비중이 크다.", "왜 응원해야 하는지 체감이 부족하다."),
        makeRubric("s2_reader_empathy_and_likability", "human_contradiction", "pressure", "인간적 모순", "호감도와 모순이 균형 있게 공존하는가?", ["단점이 인물을 입체화하되 반감만 주지 않는다", "양면성이 갈등과 선택에 연결된다"], "호감과 모순이 함께 살아 있어 캐릭터가 입체적이다.", "입체성은 있으나 단점 또는 매력이 한쪽으로 치우친다.", "인물이 평면적이거나 반감 요소만 두드러진다."),
        makeRubric("s2_reader_empathy_and_likability", "relationship_anchor", "effect", "감정 투자 앵커", "독자가 투자할 관계나 장면이 분명히 제시되는가?", ["핵심 관계가 왜 중요한지 보인다", "독자가 잃고 싶지 않은 감정 자산이 생긴다"], "관계 앵커가 선명해 감정 투자 기반이 튼튼하다.", "앵커는 있으나 인상적 장면이 부족하다.", "독자가 붙들 관계가 흐릿해 정서적 흡착력이 약하다."),
      ],
      s2_character_arc_linkage: [
        makeRubric("s2_character_arc_linkage", "arc_chain", "core", "아크 인과선", "시작-전환-끝의 내면 변화가 사건과 인과로 연결되는가?", ["내면 변화가 특정 사건에 의해 촉발된다", "결말의 선택이 초기 상태와 대비된다"], "내면 변화가 사건 선택과 치밀하게 연결된다.", "변화는 보이지만 전환의 인과가 조금 약하다.", "변화가 선언적으로만 존재하고 사건과 따로 논다."),
        makeRubric("s2_character_arc_linkage", "turning_choice", "pressure", "전환 선택의 강도", "중간 전환과 결정적 선택이 충분히 강한가?", ["전환 사건이 주인공의 믿음을 흔든다", "결정적 선택이 이전 상태를 넘어선다"], "전환과 선택이 캐릭터를 실질적으로 바꿔 놓는다.", "전환은 있으나 선택의 대가나 무게가 약하다.", "아크의 분기점이 약해 변화가 얕게 느껴진다."),
        makeRubric("s2_character_arc_linkage", "visible_proof", "effect", "변화 증거 가시성", "독자가 행동과 대사로 변화 증거를 확인할 수 있는가?", ["변화 전후의 행동 차이가 보인다", "대사나 관계 반응에서 변화가 확인된다"], "변화가 장면 증거로 또렷하게 입증된다.", "증거는 있으나 한두 장면에만 의존한다.", "변화가 설명에 머물고 독자가 체감할 장면 증거가 부족하다."),
      ],
      s2_scene_goal_conflict_change: [
        makeRubric("s2_scene_goal_conflict_change", "goal", "core", "장면 목표", "이 장면에서 인물의 즉시 목표가 선명한가?", ["장면 초반에 인물의 의도가 드러난다", "목표가 행동으로 확인된다"], "목표가 선명하고 장면 추진력을 만든다.", "목표는 보이지만 선명도나 긴급성이 약하다.", "인물이 무엇을 원하는지 흐려 장면이 떠돈다."),
        makeRubric("s2_scene_goal_conflict_change", "conflict", "pressure", "장면 충돌", "목표를 방해하는 저항이 구체적으로 존재하는가?", ["외적 또는 내적 방해가 실제 행동을 막는다", "저항이 쉽게 우회되지 않는다"], "충돌이 분명하고 긴장을 직접 발생시킨다.", "충돌은 있으나 강도나 구체성이 약하다.", "실질적인 방해가 약해 장면이 평평하게 흐른다."),
        makeRubric("s2_scene_goal_conflict_change", "change", "effect", "장면 변화", "장면 전후 상태가 분명히 달라지는가?", ["정보·감정·관계·상황 중 하나 이상이 바뀐다", "변화가 다음 장면을 밀어준다"], "전후 변화가 뚜렷해 장면의 존재 이유가 선명하다.", "변화는 있으나 체감이나 파급력이 약하다.", "장면 전후가 거의 같아 기능적 정지가 발생한다."),
      ],
      s2_scene_four_chords: [
        makeRubric("s2_scene_four_chords", "beat_balance", "core", "4코드 배치", "Action/Reaction/Setup/Deepening이 균형 있게 배치되는가?", ["행동과 반응이 분리되어 보인다", "설정과 심화가 장면 흐름 속에 자연스럽게 녹아 있다"], "네 요소가 역할을 나눠 장면 밀도를 높인다.", "요소는 있으나 일부가 겹치거나 빠진다.", "요소 배치가 흐려 장면 리듬이 단조롭다."),
        makeRubric("s2_scene_four_chords", "sequence_pressure", "pressure", "비트 전개 압력", "비트가 앞 비트의 결과로 자연스럽게 이어지는가?", ["각 코드가 직전 비트에 반응해 전개된다", "장면이 멈추지 않고 다음 비트로 넘어간다"], "비트 간 인과가 분명해 읽는 속도가 살아난다.", "전개는 되나 몇 비트가 기능적으로 겹친다.", "비트가 끊겨 보이거나 순서가 기계적으로 느껴진다."),
        makeRubric("s2_scene_four_chords", "density", "effect", "장면 밀도", "짧은 장면 안에서도 정보와 감정이 압축되어 있는가?", ["행동 외에도 감정·맥락이 함께 쌓인다", "불필요한 군더더기 없이 장면이 진전된다"], "압축도가 높아 짧아도 풍부한 장면이 된다.", "필수 요소는 있으나 압축감이 약하다.", "장면이 늘어지거나 설명이 비트 흐름을 끊는다."),
      ],
      s2_dialogue_voice_distinction: [
        makeRubric("s2_dialogue_voice_distinction", "voice_identity", "core", "보이스 개별성", "인물마다 말투와 사고 필터가 분명히 다른가?", ["화자만 보고도 누가 말하는지 짐작 가능하다", "어휘·호흡·태도가 인물 성격과 맞물린다"], "대사만으로도 인물 구분이 가능할 만큼 보이스가 살아 있다.", "차이는 있으나 몇몇 인물이 비슷하게 들린다.", "모든 인물이 비슷한 말투로 말해 개별성이 약하다."),
        makeRubric("s2_dialogue_voice_distinction", "character_pressure", "pressure", "인물성 반영도", "대사가 인물의 배경·기질·관계를 반영하는가?", ["직업·세대·기질이 어휘 선택에 드러난다", "상대에 따라 말투가 달라진다"], "대사가 인물의 삶과 관계를 자연스럽게 노출한다.", "반영은 있으나 표면적인 수준에 그친다.", "대사가 설명문처럼 들려 인물성이 묻어나지 않는다."),
        makeRubric("s2_dialogue_voice_distinction", "scene_vividness", "effect", "장면 생동감", "보이스 차이가 장면의 현실감과 리듬을 살리는가?", ["대사 충돌이 읽는 재미를 만든다", "서술 없이도 관계 긴장도가 드러난다"], "보이스 차이가 장면 생동감을 눈에 띄게 끌어올린다.", "생동감은 있으나 몇 줄 지나면 평준화된다.", "차이가 약해 대사가 기능 전달만 수행한다."),
      ],
      s2_dialogue_as_confrontation: [
        makeRubric("s2_dialogue_as_confrontation", "desire_clash", "core", "욕망 충돌", "대화 안에 서로 다른 욕망과 목표가 부딪히는가?", ["각 인물이 대화에서 얻고 싶은 것이 있다", "말이 오갈수록 힘겨루기가 생긴다"], "대사가 명확한 욕망 충돌을 만들어 낸다.", "갈등은 있으나 욕망 차이가 선명하지 않다.", "대화가 정보 교환에 머물러 대립이 약하다."),
        makeRubric("s2_dialogue_as_confrontation", "subtext", "pressure", "서브텍스트", "인물들이 속마음을 직접 말하지 않고 우회적으로 밀어붙이는가?", ["감추는 정보나 회피가 존재한다", "겉말과 속뜻의 차이가 긴장을 만든다"], "서브텍스트가 살아 있어 대사 층위가 풍부하다.", "우회는 있으나 비교적 직설적이라 긴장이 덜하다.", "속마음을 그대로 말해 대화가 평면적이다."),
        makeRubric("s2_dialogue_as_confrontation", "aftereffect", "effect", "대화 후 변화", "대화가 끝난 뒤 관계나 상황이 달라지는가?", ["대화 결과로 우위나 감정 상태가 변한다", "다음 행동을 촉발한다"], "대화가 장면 이후를 바꾸는 사건으로 작동한다.", "영향은 있으나 다음 행동을 강하게 밀지는 못한다.", "대화 후에도 관계와 상황이 거의 그대로다."),
      ],
      s3_three_act_spine: [
        makeRubric("s3_three_act_spine", "act_function", "core", "3막 기능 분리", "1막·2막·3막의 기능이 서로 다르게 작동하는가?", ["각 막이 고유한 질문과 역할을 가진다", "막 구분이 사건 기능으로 설명된다"], "세 막의 기능이 명확히 나뉘고 서로를 지지한다.", "막 구분은 있으나 일부 기능이 섞인다.", "막 구조가 흐려 사건 나열처럼 느껴진다."),
        makeRubric("s3_three_act_spine", "causal_bridge", "pressure", "막 간 인과 브리지", "막 전환이 필연적 사건으로 이어지는가?", ["막 전환 계기가 이전 사건의 결과다", "전환 후 상황이 뚜렷이 달라진다"], "막 전환이 자연스럽고 필연적으로 이어진다.", "전환은 되지만 약간 기계적이거나 급하다.", "막 전환이 뜬금없어 구조적 연결감이 약하다."),
        makeRubric("s3_three_act_spine", "macro_momentum", "effect", "거시 추진력", "전체 구조가 독자를 끝까지 끌고 갈 추진력을 갖는가?", ["각 막이 다음 막에 대한 기대를 남긴다", "구조가 점진적으로 긴장을 축적한다"], "거시 구조만 봐도 읽고 싶어지는 추진력이 있다.", "전체 흐름은 있으나 중간에 처지는 구간이 예상된다.", "구조적 동력이 약해 후반까지 버티기 어렵다."),
      ],
      s3_disturbance_two_doorways: [
        makeRubric("s3_disturbance_two_doorways", "disturbance", "core", "교란의 명료성", "Disturbance가 일상을 깨는 첫 경고음으로 작동하는가?", ["평온한 상태가 분명히 흔들린다", "이후 갈등의 씨앗이 드러난다"], "교란이 분명해 독자가 이야기 시작을 체감한다.", "교란은 있으나 파급력이 약하다.", "교란이 흐려 초반 기동력이 약하다."),
        makeRubric("s3_disturbance_two_doorways", "doorway_force", "pressure", "문턱의 강제력", "두 개의 Doorway가 되돌릴 수 없는 결정으로 작동하는가?", ["문턱을 넘기 전후 조건이 명확히 다르다", "주인공이 퇴로를 잃는다"], "두 문턱이 모두 서사의 강한 전환점이 된다.", "문턱은 있으나 강제력이나 대가가 조금 약하다.", "문턱이 사실상 선택지라 되돌릴 수 없다는 감각이 없다."),
        makeRubric("s3_disturbance_two_doorways", "trajectory", "effect", "전진 궤도", "교란과 두 문턱이 이야기의 전진 궤도를 분명히 만드는가?", ["초반-중반-후반 이동 방향이 보인다", "각 전환점이 기대를 갱신한다"], "전환점들이 이야기의 궤도를 선명하게 잡아준다.", "기능은 있으나 전체 궤도 연결이 약간 흐리다.", "전환점이 흩어져 전체 방향성이 약하다."),
      ],
      s3_hero_journey_overlay: [
        makeRubric("s3_hero_journey_overlay", "overlay_fit", "core", "오버레이 적합성", "영웅여정 비트가 작품의 3막 구조와 무리 없이 맞물리는가?", ["비트가 메인 구조를 보완한다", "형식 맞추기보다 기능 중심으로 배치된다"], "영웅여정 비트가 구조를 강화하며 자연스럽게 겹친다.", "대체로 맞지만 일부 비트가 억지로 끼워진 느낌이 있다.", "비트 오버레이가 구조를 복잡하게 만들 뿐 실익이 적다."),
        makeRubric("s3_hero_journey_overlay", "transformation_path", "pressure", "변화 여정 압력", "비트가 주인공의 변화 여정을 단계적으로 밀어붙이는가?", ["시험·동맹·시련이 내면 변화를 누적한다", "단계마다 주인공이 다른 상태로 이동한다"], "여정 비트가 캐릭터 변화를 꾸준히 압박한다.", "몇몇 비트는 작동하지만 변화 누적이 고르지 않다.", "비트가 이벤트 체크리스트처럼 흩어져 있다."),
        makeRubric("s3_hero_journey_overlay", "mythic_resonance", "effect", "상징적 울림", "오버레이가 모험과 성장의 상징적 울림을 더하는가?", ["비트가 주제나 정서와 연결된다", "모험의 단계가 독자 체감으로 남는다"], "구조와 감정 모두에 상징적 깊이를 더한다.", "효과는 있으나 장르적 장식에 가까운 부분이 있다.", "오버레이의 존재감이 약하거나 기능이 불분명하다."),
      ],
      s3_plot_points_midpoint: [
        makeRubric("s3_plot_points_midpoint", "plot_point_function", "core", "플롯 포인트 기능", "Plot Point 1·Midpoint·Plot Point 2가 각각 고유한 기능을 수행하는가?", ["세 지점의 역할이 분명히 다르다", "각 사건이 막 전환 또는 판세 전환을 만든다"], "세 지점이 구조적으로 또렷하고 기능이 겹치지 않는다.", "핵심 지점은 있으나 일부 기능이 중복된다.", "구조 포인트가 흐려 이야기의 굴절점이 약하다."),
        makeRubric("s3_plot_points_midpoint", "stakes_shift", "pressure", "판세 변화", "미드포인트 전후로 스테이크와 판세가 분명히 달라지는가?", ["중반부 이후 압박 강도나 정보 수준이 달라진다", "주인공의 대응 전략이 바뀐다"], "미드포인트가 이야기의 체급을 한 단계 끌어올린다.", "변화는 있으나 판세 전환의 충격이 조금 약하다.", "중반을 지나도 게임의 룰이나 압력이 크게 달라지지 않는다."),
        makeRubric("s3_plot_points_midpoint", "pace", "effect", "리듬 유지력", "구조 포인트들이 독서 리듬을 다시 점화하는가?", ["전환점 이후 독자가 새 기대를 가진다", "중반 정체를 해소한다"], "핵심 포인트들이 리듬 저하를 막고 몰입을 갱신한다.", "리듬 전환은 있으나 일부 구간이 여전히 늘어진다.", "전환점이 있어도 체감 리듬이 크게 달라지지 않는다."),
      ],
      s3_subplot_thematic_integration: [
        makeRubric("s3_subplot_thematic_integration", "subplot_reason", "core", "서브플롯 존재 이유", "각 서브플롯이 메인 플롯이나 테마에 기여하는가?", ["서브플롯마다 분명한 기능이 있다", "메인 갈등과의 연결점이 설명 가능하다"], "서브플롯이 존재 이유를 분명히 갖고 메인 구조를 돕는다.", "기여는 있으나 일부는 독립 에피소드처럼 느껴진다.", "서브플롯이 메인 이야기와 느슨하게 붙어 있다."),
        makeRubric("s3_subplot_thematic_integration", "integration_pressure", "pressure", "수렴 압력", "서브플롯이 후반으로 갈수록 메인 갈등과 수렴하는가?", ["클라이맥스 근처에서 서브플롯이 메인 갈등에 합류한다", "주제적 의미가 한 방향으로 모인다"], "수렴이 자연스럽고 후반 응집력을 높인다.", "수렴은 되지만 약간 기능적 봉합처럼 보인다.", "서브플롯이 따로 놀아 후반 응집력을 떨어뜨린다."),
        makeRubric("s3_subplot_thematic_integration", "theme_echo", "effect", "테마 반향", "서브플롯이 테마를 깊게 만드는 반향 효과가 있는가?", ["메인 테마를 다른 각도에서 변주한다", "대비·보완 관계가 의미를 키운다"], "서브플롯이 테마를 풍부하게 증폭한다.", "보완은 하지만 새 층위를 크게 열지는 못한다.", "테마와 연결되지 않아 복잡도만 늘린다."),
      ],
      s4_beginning_hook_entry: [
        makeRubric("s4_beginning_hook_entry", "hook_power", "core", "훅의 즉시성", "첫 장면이 독자를 바로 붙드는가?", ["첫 문단에서 질문이나 이상 신호가 생긴다", "장르 톤과 흥미 포인트가 빠르게 드러난다"], "첫 장면이 즉시 독서를 강제할 정도로 후킹된다.", "흥미 요소는 있으나 강한 흡입력까지는 부족하다.", "첫 장면이 평평해 다음 페이지로 넘길 동력이 약하다."),
        makeRubric("s4_beginning_hook_entry", "entry_clarity", "pressure", "진입 명료성", "독자가 혼란 없이 상황에 진입하면서도 궁금증을 느끼는가?", ["기본 상황 이해가 가능하다", "정보를 과잉 설명하지 않으면서 질문을 남긴다"], "명료함과 궁금증이 균형 있게 공존한다.", "이해는 되지만 미스터리나 질문의 압력이 약하다.", "혼란스럽거나 반대로 너무 평이해 진입 감도가 떨어진다."),
        makeRubric("s4_beginning_hook_entry", "next_page_force", "effect", "다음 페이지 강제력", "첫 장면이 다음 장면으로 독자를 밀어주는가?", ["브리지나 미해결 요소가 남는다", "독자가 확인하고 싶은 정보가 생긴다"], "다음 페이지를 자연스럽게 강제한다.", "이어 읽을 이유는 있으나 절박함은 덜하다.", "첫 장면이 닫혀 있어 계속 읽을 강한 이유가 부족하다."),
      ],
      s4_beginning_world_opposition: [
        makeRubric("s4_beginning_world_opposition", "world_rules", "core", "세계 규칙 제시", "독자가 알아야 할 세계 규칙이 사건 속에서 전달되는가?", ["설정이 설명문보다 장면 동작 속에 들어 있다", "최소한의 규칙만 선별되어 제시된다"], "세계 규칙이 자연스럽고 효율적으로 전달된다.", "핵심 규칙은 보이지만 전달 방식이 조금 설명적이다.", "규칙 전달이 과잉 설명이거나 너무 부족해 혼란을 준다."),
        makeRubric("s4_beginning_world_opposition", "opposition_seed", "pressure", "대립 씨앗", "초반부터 주인공을 압박할 저항이 암시되거나 등장하는가?", ["반대세력 또는 저항 구조가 보인다", "초반 안에 충돌 방향이 제시된다"], "대립의 씨앗이 초반부터 분명히 심긴다.", "씨앗은 있으나 강한 존재감은 아직 약하다.", "대립 방향이 늦게 드러나 초반 긴장이 약하다."),
        makeRubric("s4_beginning_world_opposition", "reader_clarity", "effect", "독자 이해도", "독자가 혼란 없이 세계와 대립 축을 따라갈 수 있는가?", ["초반 질문과 필수 정보가 구분된다", "혼란이 의도인지 정보 누락인지 구별된다"], "이해와 궁금증의 균형이 좋아 독자가 안정적으로 따라간다.", "대체로 이해되지만 일부 정보 배치가 거칠다.", "독자가 기초 상황을 놓쳐 몰입보다 해석에 에너지를 쓴다."),
      ],
      s4_middle_escalation_pressure: [
        makeRubric("s4_middle_escalation_pressure", "escalation_steps", "core", "상승 단계", "중반부 갈등이 단계적으로 더 어려워지는가?", ["사건마다 압박 수준이 올라간다", "주인공이 이전 방식으로는 버티기 어려워진다"], "갈등이 계단식으로 상승해 중반부 동력이 강하다.", "상승은 있으나 단계 차이가 조금 약하다.", "중반부 갈등이 반복적으로 느껴져 정체감이 생긴다."),
        makeRubric("s4_middle_escalation_pressure", "resource_depletion", "pressure", "자원 고갈 압력", "주인공의 선택지와 자원이 점점 줄어드는가?", ["안전망·정보·관계가 소진된다", "주인공 대응이 점점 비용을 치른다"], "주인공이 점점 궁지로 몰리는 감각이 선명하다.", "곤란은 커지지만 자원 고갈 체감은 다소 약하다.", "어려움은 늘어도 주인공이 별 손실 없이 버티는 느낌이다."),
        makeRubric("s4_middle_escalation_pressure", "mid_section_drive", "effect", "중반 추진력", "중반부가 지루해지지 않고 계속 읽히는가?", ["사건 압박이 다음 전개를 기대하게 만든다", "중반 각 장면의 존재 이유가 드러난다"], "중반부가 체감상 가장 역동적인 구간으로 작동한다.", "읽히기는 하지만 몇 구간은 늘어진다.", "중반부가 반복과 정체로 몰입을 떨어뜨린다."),
      ],
      s4_middle_complications_reversals: [
        makeRubric("s4_middle_complications_reversals", "complication_quality", "core", "합병증 품질", "문제가 단순 반복이 아니라 새로운 합병증으로 확장되는가?", ["기존 문제에 새로운 변수나 비용이 붙는다", "상황이 더 복잡해지며 선택이 어려워진다"], "합병증이 자연스럽게 서사를 더 어렵고 풍부하게 만든다.", "확장은 있으나 변수의 새로움이 조금 약하다.", "문제가 반복될 뿐 구조적 복잡도가 늘지 않는다."),
        makeRubric("s4_middle_complications_reversals", "reversal_force", "pressure", "반전 강도", "반전이 판세나 독자 기대를 실질적으로 뒤집는가?", ["반전 전후의 상태 차이가 분명하다", "반전이 이후 전략이나 관계를 바꾼다"], "반전이 판세를 눈에 띄게 뒤집어 긴장을 재점화한다.", "반전은 있으나 영향 범위가 제한적이다.", "반전이 예상 가능하거나 영향이 미미하다."),
        makeRubric("s4_middle_complications_reversals", "surprise_logic", "effect", "놀라움과 인과", "반전이 놀랍지만 뜬금없지는 않은가?", ["사후에 납득 가능한 단서가 있다", "충격과 필연성이 함께 유지된다"], "놀라움과 인과가 균형 있게 맞물린다.", "의도는 좋지만 약간 억지스럽거나 예측 가능하다.", "반전이 작위적이거나 너무 뻔해 효과가 약하다."),
      ],
      s4_ending_knockout_resolution: [
        makeRubric("s4_ending_knockout_resolution", "climax_resolution", "core", "클라이맥스 해결", "최종 결전이 핵심 갈등을 정면으로 해결하는가?", ["주인공이 주요 갈등과 직접 맞선다", "해결 방식이 이야기 전반의 인과를 회수한다"], "클라이맥스가 갈등 해결의 정점으로 강하게 작동한다.", "해결은 되지만 감정 또는 인과의 압력이 조금 약하다.", "핵심 갈등 해결이 우회되거나 힘이 빠진다."),
        makeRubric("s4_ending_knockout_resolution", "cost_and_choice", "pressure", "선택과 대가", "결말의 승패가 충분한 선택과 대가 위에 놓여 있는가?", ["주인공이 값비싼 선택을 한다", "승리 또는 실패의 대가가 분명하다"], "선택과 대가가 결말의 무게를 확실히 만든다.", "대가는 있으나 아프게 체감되지는 않는다.", "결말이 쉽게 풀려 무게감과 희생이 약하다."),
        makeRubric("s4_ending_knockout_resolution", "closure", "effect", "감정적 봉합", "결말이 독자의 감정선을 적절히 봉합하는가?", ["갈등 해결 후 감정 정리가 있다", "결말의 의미가 독자에게 전달된다"], "해결과 감정 정리가 함께 이뤄져 만족도가 높다.", "봉합은 되지만 약간 급하거나 얇다.", "사건은 끝났지만 감정적으로 미완의 느낌이 크다."),
      ],
      s4_last_page_resonance: [
        makeRubric("s4_last_page_resonance", "final_image", "core", "마지막 이미지", "마지막 장면의 이미지나 문장이 기억에 남는가?", ["마지막 한 컷이 정서나 의미를 응축한다", "종결감 있는 이미지가 남는다"], "마지막 이미지가 작품 전체를 다시 울리게 만든다.", "이미지는 있으나 강한 인상까지는 부족하다.", "마지막 장면이 무난하게 끝나 기억점이 약하다."),
        makeRubric("s4_last_page_resonance", "theme_afterglow", "pressure", "테마 잔향", "마지막 페이지가 작품의 질문이나 테마를 잔향으로 남기는가?", ["결말 후에도 생각할 질문이나 감정이 남는다", "테마가 직접 설교되지 않고 스며든다"], "테마가 은근하지만 분명한 잔향으로 남는다.", "의미는 전달되지만 다소 직설적이거나 약하다.", "끝나고 나서 곱씹을 의미의 잔향이 약하다."),
        makeRubric("s4_last_page_resonance", "lasting_feeling", "effect", "지속 감정", "책장을 덮은 뒤에도 감정이 지속되는가?", ["독자의 감정 상태를 남기는 장치가 있다", "마지막 문장이 작품 톤과 잘 맞는다"], "독서 종료 후에도 감정이 오래 남는다.", "감정은 남지만 강한 지속성까지는 부족하다.", "끝맺음이 기능적이어서 감정 잔존감이 약하다."),
      ],
      s5_revision_triage: [
        makeRubric("s5_revision_triage", "priority_sorting", "core", "수정 우선순위", "수정해야 할 문제들이 구조적 우선순위에 따라 정리되는가?", ["거시 문제와 미시 문제를 구분한다", "먼저 고칠 문제와 나중에 다듬을 문제를 나눈다"], "우선순위가 분명해 퇴고 방향이 선명하다.", "정리는 되어 있으나 우선순위 기준이 조금 흐리다.", "문제 나열만 있고 무엇부터 손댈지 보이지 않는다."),
        makeRubric("s5_revision_triage", "fatal_issue_focus", "pressure", "치명상 집중도", "문장보다 뼈대 문제를 먼저 다루도록 설계되어 있는가?", ["플롯·아크·장면 기능 문제를 먼저 잡는다", "표현 다듬기는 후순위로 밀린다"], "가장 치명적인 구조 문제에 초점이 잘 맞춰진다.", "큰 문제도 보지만 세부 수정으로 자주 새어 나간다.", "치명상보다 문장 다듬기에 에너지를 먼저 쓴다."),
        makeRubric("s5_revision_triage", "rewrite_path", "effect", "재작성 경로", "수정 계획이 실제 재작성 행동으로 이어질 수 있는가?", ["수정 항목마다 실행 액션이 있다", "리라이트 순서가 떠오른다"], "퇴고 계획이 바로 실행 가능한 작업 목록이 된다.", "계획은 있으나 실행 단위가 다소 모호하다.", "판단은 있지만 실제 리라이트 경로가 보이지 않는다."),
      ],
      s5_common_problem_cures: [
        makeRubric("s5_common_problem_cures", "diagnosis_match", "core", "문제-처방 정합성", "문제 진단과 처방이 정확히 짝지어지는가?", ["증상에 맞는 처방이 연결된다", "엉뚱한 수술로 새 문제를 만들지 않는다"], "진단과 처방이 정합적으로 맞물린다.", "대체로 맞지만 몇 처방은 범용적이다.", "문제와 처방이 어긋나 효과가 불확실하다."),
        makeRubric("s5_common_problem_cures", "cause_level", "pressure", "원인 수준 접근", "표면 증상보다 근본 원인을 겨냥하는가?", ["문제의 상위 원인을 짚는다", "결과보다 구조적 원인을 다룬다"], "근본 원인을 겨냥해 재발 가능성을 낮춘다.", "원인도 보지만 일부는 증상 처방에 그친다.", "표면만 다뤄 문제의 재발 가능성이 높다."),
        makeRubric("s5_common_problem_cures", "repair_efficiency", "effect", "수정 효율", "제시된 처방이 효율적으로 원고를 개선할 수 있는가?", ["적은 수정으로 큰 효과를 노린다", "우선순위 높은 처방이 앞에 온다"], "효율 좋은 처방이 골라져 수정 대비 효과가 크다.", "유효하지만 비용 대비 효과가 다소 불균형하다.", "처방이 과도하거나 산발적이라 수정 효율이 낮다."),
      ],
      s5_show_tell_and_precision_tools: [
        makeRubric("s5_show_tell_and_precision_tools", "show_tell_control", "core", "Show/Tell 조절", "보여주기와 말해주기의 비율이 목적에 맞게 조절되는가?", ["장면화가 필요한 부분은 보여준다", "요약이 필요한 부분은 압축해서 말한다"], "보여주기와 말해주기가 목적에 맞게 정확히 배치된다.", "대체로 맞지만 몇 부분은 장면화 또는 요약이 과하다.", "모두 보여주거나 모두 설명해 리듬과 집중도가 무너진다."),
        makeRubric("s5_show_tell_and_precision_tools", "language_precision", "pressure", "표현 정밀도", "문장이 모호한 추상어 대신 구체 행동과 감각으로 쓰이는가?", ["추상 명사보다 행동·감각 표현이 많다", "수식보다 핵심 정보가 먼저 보인다"], "표현이 정밀해 독자가 장면을 바로 잡는다.", "의미는 통하지만 일부 문장이 흐리거나 일반적이다.", "표현이 모호하고 추상적이라 장면이 잘 보이지 않는다."),
        makeRubric("s5_show_tell_and_precision_tools", "revision_portability", "effect", "도구 이식성", "정리한 규칙이 다른 원고에도 반복 적용 가능한가?", ["재사용 가능한 문장 규칙이 정리된다", "다음 퇴고에 옮겨 쓸 기준이 남는다"], "툴킷이 다른 장면에도 바로 이식 가능하다.", "일부는 재사용 가능하지만 사례 의존성이 크다.", "이번 원고에만 한정되어 재사용 가치가 낮다."),
      ],
    };

    function attachRubricsToDrills(targetDrills) {
      targetDrills.forEach((drill) => {
        drill.rubrics = deepCopy(koRubricMap[drill.id] || []);
      });
    }

    function buildEnglishRubricLabel(dimension) {
      const map = {
        core: "Core Function",
        pressure: "Pressure",
        effect: "Reader Effect",
      };
      return map[dimension] || "Rubric";
    }

    function buildEnglishRubricQuestion(dimension, drill) {
      const title = drill.title || "this node";
      const map = {
        core: `Does the draft clearly deliver the core function of ${title}?`,
        pressure: `Does ${title} create enough pressure, cost, or escalation on the page?`,
        effect: `Does ${title} produce a visible reader-facing effect in the draft?`,
      };
      return map[dimension] || `Does the draft execute ${title} clearly?`;
    }

    function buildEnglishRubricLookFor(dimension) {
      const map = {
        core: ["A concrete craft decision is visible on the page", "The node is executable without guesswork"],
        pressure: ["Conflict, cost, urgency, or irreversible movement increases", "The node meaningfully narrows easy options"],
        effect: ["A reader can feel the scene, structure, or emotional effect changing", "The node leaves a clear consequence or forward pull"],
      };
      return map[dimension] || ["Concrete evidence in the draft", "A visible story effect"];
    }

    function buildEnglishRubricScoreGuide(dimension) {
      const map = {
        core: makeScoreGuide("The node's core function is explicit and fully usable.", "The function is present but somewhat soft or abstract.", "The node's function is vague or barely visible."),
        pressure: makeScoreGuide("Pressure or escalation is clear and materially changes the scene or structure.", "Pressure exists but remains limited or inconsistent.", "Pressure is weak, generic, or missing."),
        effect: makeScoreGuide("The reader-facing effect is immediate, distinct, and memorable.", "There is some effect, but it lands lightly.", "The effect is too faint to register clearly."),
      };
      return map[dimension] || makeScoreGuide("Strong", "Partial", "Weak");
    }
    ...
    function localizeDrill(drill) {
      ...
      drill.rubrics = (drill.rubrics || []).map((rubric) => ({
        ...rubric,
        label: normalizeSentence(replaceUiTerms(rubric.label || "")),
        question: normalizeSentence(replaceUiTerms(rubric.question || "")),
        lookFor: (rubric.lookFor || []).map((item) => normalizeSentence(replaceUiTerms(item))),
        scoreGuide: {
          high: normalizeSentence(replaceUiTerms(rubric.scoreGuide?.high || "")),
          mid: normalizeSentence(replaceUiTerms(rubric.scoreGuide?.mid || "")),
          low: normalizeSentence(replaceUiTerms(rubric.scoreGuide?.low || "")),
        },
      }));
    }
    ...
    function buildEnglishDataset(koData) {
      ...
      enData.drills.forEach((drill) => {
        ...
        drill.rubrics = (drill.rubrics || []).map((rubric) => ({
          ...rubric,
          label: buildEnglishRubricLabel(rubric.dimension),
          question: buildEnglishRubricQuestion(rubric.dimension, drill),
          lookFor: buildEnglishRubricLookFor(rubric.dimension),
          scoreGuide: buildEnglishRubricScoreGuide(rubric.dimension),
        }));
      });
    ...
    const analysisSetupEl = document.getElementById("analysisSetup");
    ...
    function loadAnalysisContext() {
      try {
        ...
        analysisContext = createDefaultAnalysisContext();
      }
    }
    ...
    function getGenreOptions() {
      return [
        { value: "unknown", label: t("optionGenreUnknown") },
        { value: "modern_fantasy", label: t("optionGenreModernFantasy") },
        { value: "fantasy", label: t("optionGenreFantasy") },
        { value: "romance_fantasy", label: t("optionGenreRomanceFantasy") },
        { value: "romance", label: t("optionGenreRomance") },
        { value: "thriller", label: t("optionGenreThriller") },
        { value: "drama", label: t("optionGenreDrama") },
        { value: "action", label: t("optionGenreAction") },
        { value: "etc", label: t("optionGenreEtc") },
      ];
    }

    function getDraftStageOptions() {
      return [
        { value: "planning", label: t("optionDraftPlanning") },
        { value: "first", label: t("optionDraftFirst") },
        { value: "rewrite", label: t("optionDraftRewrite") },
        { value: "polish", label: t("optionDraftPolish") },
      ];
    }

    function getNarrativePovOptions() {
      return [
        { value: "unknown", label: t("optionPovUnknown") },
        { value: "first_person", label: t("optionPovFirst") },
        { value: "third_limited", label: t("optionPovThirdLimited") },
        { value: "third_omniscient", label: t("optionPovThirdOmni") },
      ];
    }

    function getPresetOptions() {
      return Object.values(editorialPresets).map((preset) => ({
        value: preset.id,
        label: t(preset.labelKey),
        hint: t(`${preset.labelKey.replace("Label", "Hint")}`),
      }));
    }

    function getOptionLabel(options, value) {
      const matched = options.find((item) => item.value === value);
      return matched ? matched.label : options[0].label;
    }

    function getSelectedPresetConfig() {
      return editorialPresets[analysisContext.presetId] || editorialPresets.balanced;
    }

    function buildChecksForRequest(selected) {
      return (selected?.rubrics || []).map((rubric) => ({
        id: rubric.id,
        label: rubric.label,
        question: rubric.question,
        lookFor: Array.isArray(rubric.lookFor) ? rubric.lookFor : [],
        scoreGuide: {
          high: rubric.scoreGuide?.high || "",
          mid: rubric.scoreGuide?.mid || "",
          low: rubric.scoreGuide?.low || "",
        },
      }));
    }

    function buildMetaForRequest(selected) {
      return {
        lang: uiLang,
        version: "v3",
        nodeId: selected.id,
        ...
        genre: getOptionLabel(getGenreOptions(), analysisContext.genre),
        draftStage: getOptionLabel(getDraftStageOptions(), analysisContext.draftStage),
        narrativePOV: getOptionLabel(getNarrativePovOptions(), analysisContext.narrativePOV),
        authorGoal: String(analysisContext.authorGoal || "").trim(),
        mustKeep: parseMustKeepText(analysisContext.mustKeepText),
      };
    }

    function buildPresetForRequest() {
      const preset = getSelectedPresetConfig();
      return {
        id: preset.id,
        label: t(preset.labelKey),
        editorRole: preset.editorRole,
        toneInstruction: preset.toneInstruction,
        suggestionInstruction: preset.suggestionInstruction,
      };
    }

    function renderAnalysisSetup() {
      if (!analysisSetupEl) return;
      const selected = getSelectedNode();
      if (!selected) {
        analysisSetupEl.innerHTML = "";
        return;
      }

      const rubrics = Array.isArray(selected.rubrics) ? selected.rubrics : [];
      const genreLabel = getOptionLabel(getGenreOptions(), analysisContext.genre);
      const draftStageLabel = getOptionLabel(getDraftStageOptions(), analysisContext.draftStage);
      const povLabel = getOptionLabel(getNarrativePovOptions(), analysisContext.narrativePOV);
      const preset = getSelectedPresetConfig();
      const presetOptions = getPresetOptions();
      const rubricItems = rubrics.map((rubric) => `
        <li>
          <strong>${escapeHtml(rubric.label)}</strong>
          <div class="analysis-guide">
            <p><strong>${escapeHtml(t("rubricQuestion"))}:</strong> ${escapeHtml(rubric.question)}</p>
            <p><strong>${escapeHtml(t("rubricLookFor"))}:</strong> ${escapeHtml((rubric.lookFor || []).join(" / "))}</p>
            <p><strong>${escapeHtml(t("rubricScoreGuide"))}:</strong> H ${escapeHtml(rubric.scoreGuide?.high || "-")} / M ${escapeHtml(rubric.scoreGuide?.mid || "-")} / L ${escapeHtml(rubric.scoreGuide?.low || "-")}</p>
          </div>
        </li>
      `).join("");

      analysisSetupEl.innerHTML = `
        <div class="analysis-setup-header">
          <h3>${escapeHtml(t("analysisSetupTitle"))}</h3>
          <p>${escapeHtml(t("analysisSetupHint"))}</p>
        </div>
        <details class="analysis-card">
          <summary>
            <span class="analysis-summary-title">${escapeHtml(t("checksCardTitle"))}</span>
            <span class="analysis-summary-copy">${escapeHtml(t("checksCardSummary", { count: rubrics.length }))}</span>
          </summary>
          <div class="analysis-body">
            <p class="analysis-note">${escapeHtml(t("checksCardHint"))}</p>
            <ol class="analysis-list">${rubricItems}</ol>
          </div>
        </details>
        <details class="analysis-card">
          <summary>
            <span class="analysis-summary-title">${escapeHtml(t("metaCardTitle"))}</span>
            <span class="analysis-summary-copy">${escapeHtml(t("metaCardSummary", { draftStage: draftStageLabel, genre: genreLabel, narrativePOV: povLabel }))}</span>
          </summary>
          <div class="analysis-body">
            <div class="analysis-grid">
              <div class="analysis-field">
                <label for="analysisGenre">${escapeHtml(t("genreLabel"))}</label>
                <select id="analysisGenre">${getGenreOptions().map((option) => `<option value="${escapeHtml(option.value)}"${analysisContext.genre === option.value ? " selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</select>
              </div>
              <div class="analysis-field">
                <label for="analysisDraftStage">${escapeHtml(t("draftStageLabel"))}</label>
                <select id="analysisDraftStage">${getDraftStageOptions().map((option) => `<option value="${escapeHtml(option.value)}"${analysisContext.draftStage === option.value ? " selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</select>
              </div>
              <div class="analysis-field">
                <label for="analysisNarrativePov">${escapeHtml(t("narrativePOVLabel"))}</label>
                <select id="analysisNarrativePov">${getNarrativePovOptions().map((option) => `<option value="${escapeHtml(option.value)}"${analysisContext.narrativePOV === option.value ? " selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</select>
              </div>
            </div>
            <div class="analysis-field">
              <label for="analysisAuthorGoal">${escapeHtml(t("authorGoalLabel"))}</label>
              <textarea id="analysisAuthorGoal" class="exercise-input" placeholder="${escapeHtml(t("authorGoalPlaceholder"))}">${escapeHtml(analysisContext.authorGoal || "")}</textarea>
            </div>
            <div class="analysis-field">
              <label for="analysisMustKeep">${escapeHtml(t("mustKeepLabel"))}</label>
              <textarea id="analysisMustKeep" class="exercise-input" placeholder="${escapeHtml(t("mustKeepPlaceholder"))}">${escapeHtml(analysisContext.mustKeepText || "")}</textarea>
            </div>
          </div>
        </details>
        <details class="analysis-card">
          <summary>
            <span class="analysis-summary-title">${escapeHtml(t("presetCardTitle"))}</span>
            <span class="analysis-summary-copy">${escapeHtml(t("presetCardSummary", { label: t(preset.labelKey) }))}</span>
          </summary>
          <div class="analysis-body">
            <div class="analysis-field">
              <label for="analysisPreset">${escapeHtml(t("presetCardTitle"))}</label>
              <select id="analysisPreset">${presetOptions.map((option) => `<option value="${escapeHtml(option.value)}"${analysisContext.presetId === option.value ? " selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</select>
            </div>
            <p id="analysisPresetHint" class="analysis-note">${escapeHtml(t(`${preset.labelKey.replace("Label", "Hint")}`))}</p>
          </div>
        </details>
      `;

      const genreEl = document.getElementById("analysisGenre");
      const draftStageEl = document.getElementById("analysisDraftStage");
      const povEl = document.getElementById("analysisNarrativePov");
      const authorGoalEl = document.getElementById("analysisAuthorGoal");
      const mustKeepEl = document.getElementById("analysisMustKeep");
      const presetEl = document.getElementById("analysisPreset");

      if (genreEl) {
        genreEl.addEventListener("change", () => {
          analysisContext.genre = genreEl.value;
          saveAnalysisContext();
          renderAnalysisSetup();
        });
      }
      if (draftStageEl) {
        draftStageEl.addEventListener("change", () => {
          analysisContext.draftStage = draftStageEl.value;
          saveAnalysisContext();
          renderAnalysisSetup();
        });
      }
      if (povEl) {
        povEl.addEventListener("change", () => {
          analysisContext.narrativePOV = povEl.value;
          saveAnalysisContext();
          renderAnalysisSetup();
        });
      }
      if (authorGoalEl) {
        autoResizeTextarea(authorGoalEl);
        authorGoalEl.addEventListener("input", () => {
          autoResizeTextarea(authorGoalEl);
          analysisContext.authorGoal = authorGoalEl.value;
          saveAnalysisContext();
        });
      }
      if (mustKeepEl) {
        autoResizeTextarea(mustKeepEl);
        mustKeepEl.addEventListener("input", () => {
          autoResizeTextarea(mustKeepEl);
          analysisContext.mustKeepText = mustKeepEl.value;
          saveAnalysisContext();
        });
      }
      if (presetEl) {
        presetEl.addEventListener("change", () => {
          analysisContext.presetId = presetEl.value;
          saveAnalysisContext();
          renderAnalysisSetup();
        });
      }
    ...
    function buildStructuredExerciseText(selected, values) {
      ...
      lines.push(uiLang === "en" ? "[NODE INPUT]" : "[노드 입력]");
      lines.push("");
    ...
    function applyAutoCompletionFromResponse(selectedNodeId, payload) {
      if (!payload || typeof payload.overallScore !== "number") return;

      const progress = ensureProgress(selectedNodeId);
      progress.lastScore = Number(payload.overallScore);
      progress.autoDone = Number(payload.overallScore) >= AUTO_DONE_THRESHOLD;
      progress.lastUpdatedAt = new Date().toISOString();
      saveCurriculumState();
    ...
    async function runAnalysis() {
      ...
      const payload = {
        manuscript,
        preset: buildPresetForRequest(),
        checks: buildChecksForRequest(selected),
        turnstileToken,
        meta: buildMetaForRequest(selected),
      };
