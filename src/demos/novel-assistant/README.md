...
정적 페이지(`index.html`)는 최초 진입 시 `학습 모드`와 `서퍼 모드`를 선택할 수 있도록 구성되어 있습니다.
- 학습 모드: Plot & Structure 기반의 구조형 학습 로드맵 + 노드별 피드백 루프
- 서퍼 모드: 구조 입력 기반 자동 프리셋 + 3막 텐션 그래프(1~10 드래그) 편집 도구

학습 모드에서는 선택한 노드의 입력 템플릿 + 원고 본문을 합성해 프록시 API로 전달합니다.
