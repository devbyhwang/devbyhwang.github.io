...
import { App } from "./App";
import type { Catalog, Game } from "../domain/types";
...
describe("App", () => {
  ...
  it("safe 카드만 1/2/3 숫자 배지이고 rising/new 카드는 텍스트 배지다", () => {
    ...
    render(<App catalog={dataset(catalog)} />);
  ...
  it("인원 단계에서 막히면 인원을 줄이라고 안내한다", async () => {
    render(<App catalog={dataset([syntheticGame("players", { players: 1 })])} />);
  ...
  it("분위기 단계에서 막히면 분위기를 바꾸라고 안내한다", () => {
    render(<App catalog={dataset([syntheticGame("vibe", { players: 5, healing: 0.2 })])} />);
    expect(screen.queryAllByRole("article").length).toBe(0);
  ...
  it("필터 통과 후 결과가 없으면 특정 조건을 지목하지 않는다", () => {
    render(<App catalog={dataset([syntheticGame("other", { channels: 2 })])} />);
    expect(screen.queryAllByRole("article").length).toBe(0);
  ...
  it("신작 카드의 출시 경과는 시스템 시간이 아닌 수집 시각을 사용한다", () => {
    ...
    try {
      render(<App catalog={dataset([syntheticGame("catalog-date", { channels: 2, isNewRelease: true })])} />);
