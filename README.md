...
## 소개
- DevByHwang: 개발 기록, 실험, 외주 워크플로우를 다루는 개발자 브랜드
- Underground Novel: 소설과 노트를 쌓아가는 글쓰기 브랜드
...
- `src/demos/`: DevByHwang Playground 데모 정적 파일(빌드 시 `/devbyhwang/playground/`로 복사)
- `src/_data/studio.js`: Playground 카드와 DevByHwang 소개 카드 데이터
- `src/_data/brands.js`: 브랜드 네비게이션/테마 설정
- `src/_data/site.js`: 사이트 메타데이터와 Google Ads 설정
...
- Playground 하단 광고는 `GOOGLE_ADS_PLAYGROUND_BOTTOM_SLOT`로 별도 설정합니다.
...
- 콘텐츠 경로는 `LICENSE-CONTENT`의 Scope를 따릅니다.
  ...
  - 포함: Playground 전용 자산(`src/assets/embercraft.js`, `src/assets/embercraft-preview.png`, `src/assets/nemo-game-preview.png`)
  - 포함: 프로필 이미지(`src/assets/profile.png`, `src/assets/dodoes-profile.png`)
- 그 외 레이아웃/테마/템플릿/빌드 코드는 MIT를 따릅니다.
