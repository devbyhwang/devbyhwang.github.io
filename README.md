...
- `src/demos/`: Playground 데모 정적 파일(빌드 시 `/playground/`로 복사)
...
- 기본 카테고리: `devlog` (`devlog | freelance | games` 권장)
...
- front matter의 `views`는 선택 시드값이며, 설정하지 않으면 `0`으로 처리됩니다.
- 인기글 정렬 기준은 `effective views = seed views + runtime views` 입니다.
